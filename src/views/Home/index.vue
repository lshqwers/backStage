    ...
    >
      ...
      >
        <template slot-scope="scope">
          <input v-model="scope.row.date" v-show="scope.row.isEdit" />
      ...
      <el-table-column prop="city" label="市区" align="center" v-if="colData[3].istrue">
      </el-table-column>
      <el-table-column prop="sex" label="性别" align="center" v-if="colData[4].istrue">
      </el-table-column>
      <el-table-column label="操作" width="120" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="() => editEvent(scope.row)"
            >编辑</el-button
          >
          <el-button type="text" size="small" @click="() => saveEvent(scope.row)"
            >保存</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    changeGroup(groupValues) {
      ...
      console.log(
        JSON.parse(JSON.stringify(this.colData)),
        "12JSON.parse(JSON.stringify(this.colData))"
      );
    },
    resect() {
      // this.colOptions = []
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        if (index === 0) {
          sums[index] = (() => {
            let el = (
              <div class="count_row" style="border-bottom: 1px solid red; width: 100%;">
    ...
    editEvent(scopeValue) {
      scopeValue.isEdit = !scopeValue.isEdit;
    },
    saveEvent(scopeValue) {
      scopeValue.isEdit = false;
    },
    /**
     * @type: 单击单元格可编辑功能]
     * **/
    ...
    changeInput(row, column, cell, event) {
      let nowObj = column.property; // 获取当前点击的键
      this.nowObj = nowObj;
      this.rowData = row;
      ...
      return (this.showEdit[index][nowObj] = !this.showEdit[index][nowObj]);
    },
    inputBlur(row) {
      // 保存
      // 初始化
      this.setShowEdit(); // 如果新增表格报undefined, 在新增表格的地方可在此触发该事件
    },
  },
  mounted() {
  ...
  directives: {
    focus: {
      ...
      inserted: function (el) {
        el.getElementsByClassName("input")[0].focus();
        // el.focus()
      },
    },
  },
