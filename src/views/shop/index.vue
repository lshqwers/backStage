               ...
               thead: [
                   {
                       ...
                       callback: (row, prop)=> {
                            // console.log(row,12,prop, row[prop])
                               const data = this.parking_type.filter(item => item.value == row.type)
                            //    console.log(row.type, 12, data, 'dataCallBack')
                   ...
                   { 
                        ...
                        callback: (row, prop) => {
                            ...
                            // </el-switch>
                            console.log(row.status, 'row', row.id)
                            return row
                   ...
                   { label: '查看位置', prop: 'lnglat', type: 'lngFun', width: '260px' },
                   {
                     ...
                     width: '60px',
                     callback: (row, prop) => {
                        const dealData = this.parking_sex.filter(item => item.value === row[prop])
