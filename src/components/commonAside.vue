<template>
  <el-container class="container">
    <el-menu
      ...
      :default-openeds="
        menuCurrentIndex ? JSON.parse(JSON.stringify(menuCurrentIndex)) : []
      "
    >
      ...
      <template v-for="item in menuData">
        ...
        <el-submenu :index="item.label" v-if="item.children" :key="item.label">
          ...
          <el-menu-item-group>
            <el-menu-item
              v-for="itemVal in item.children"
              :key="itemVal.path"
              :index="itemVal.path"
              @click="jumpRoute(itemVal, 'child')"
            >
...
<script>
import { isCollapseMenu2 } from "../store/index";
  ...
  methods: {
    ...
    jumpRoute(val, type) {
      if (this.valString) {
        ...
        // let pathUrl = []
        // this.tagData
        this.tagData = this.tagData.filter((item) => {
          console.log(item, "item");
          if (item !== (this.tag && this.tag.label)) {
            return item;
          }
        });
        ...
        if (val.label !== "首页" && this.tagData.indexOf(val.label) == -1) {
          ...
          this.tagAggregate.push(val);
        } else {
          // 写在这里面会一直添加
          this.tag = "";
        }
        //  this.tag = ""; 这里就不会一直添加
        // const newTags = ( this.tagAggregate || []).map((item) => {
        //    if( val.label !== '首页' && item.label !== val.label ){
        //       this.tagAggregate.push(val)
        //    }
        // })
        console.log(
          this.tagData,
          "this.tagDatathis.tagData",
          this.tagAggregate,
          "123",
          val.label
        );
      ...
      this.$store.state.tab.meunData = this.tagAggregate;
      // 通过commit来修改mutations
      this.$store.commit("SETTABLIst", this.tagAggregate);
      // console.log( this.tagData, 12, this.$store.state.tab.meunData)
      if (type) {
        this.$router.push({
          //path: val.path 这种会报错
          // name: 'path2',
      ...
      // 调用store中的selectMenu这个方法
      this.$store.commit("selectMenu", val);
    },
    // ...mapMutations([isCollapseMenu2])
  },
  // computed计算属性
  computed: {
    // 没有子项
